...
class VAE(nn.Module):
    def __init__(self, num_aa_type, dim_latent_vars, dim_msa_vars, 
                 encoder_num_hidden_units, decoder_num_hidden_units):
        ...
        ## num of hidden neurons in encoder networks
        self.encoder_num_hidden_units = encoder_num_hidden_units

        ## num of hidden neurons in decoder networks
        self.decoder_num_hidden_units = decoder_num_hidden_units

        ## encoder
        self.encoder_linears = nn.ModuleList()
        self.encoder_linears.append(nn.Linear(dim_msa_vars, encoder_num_hidden_units[0]))
        for i in range(1, len(encoder_num_hidden_units)):
            self.encoder_linears.append(nn.Linear(encoder_num_hidden_units[i-1], encoder_num_hidden_units[i]))
        self.encoder_mu = nn.Linear(encoder_num_hidden_units[-1], dim_latent_vars, bias = True)
        self.encoder_logsigma = nn.Linear(encoder_num_hidden_units[-1], dim_latent_vars, bias = True)

        ## decoder
        self.decoder_linears = nn.ModuleList()
        self.decoder_linears.append(nn.Linear(dim_latent_vars, decoder_num_hidden_units[0]))
        for i in range(1, len(decoder_num_hidden_units)):
            self.decoder_linears.append(nn.Linear(decoder_num_hidden_units[i-1], decoder_num_hidden_units[i]))
        self.decoder_linears.append(nn.Linear(decoder_num_hidden_units[-1], dim_msa_vars))
